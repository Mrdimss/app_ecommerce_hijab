...
class ProductTransactionResource extends Resource
{
            ...
            ->schema([
                ...
                Forms\Components\Wizard::make([
                    ...
                    ->schema([
                        ...
                        Grid::make(2)
                        ->schema([
                            Forms\Components\Select::make('hijab_id')
                            ->relationship('hijab', 'name')
                            ->searchable()
                            ->preload()
                            ->required()
                            ->live()
                            ->afterStateUpdated(function ($state, callable $get, callable $set){
                                $hijab = Hijab::find($state);
                                $price = $hijab ? $hijab->price : 0;
                                ...
                                $sizes = $hijab ? $hijab->sizes->pluck('size', 'id')->toArray() : [];
                                $set('hijab_sizes', $sizes);
                            })
                            ->afterStateHydrated(function (callable $get, callable $set, $state){
                                $hijabId = $state;
                                if ($hijabId){
                                    $hijab = Hijab::find($hijabId);
                                    $sizes = $hijab ? $hijab->sizes->pluck('size', 'id')->toArray() : [];
                                    $set('hijab_sizes', $sizes);
                                }
                            }),

                            Forms\Components\Select::make('hijab_size')
                            ->label('Hijab Size')
                            ->options(function (callable $get){
                                $sizes = $get('hijab_sizes');
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                //
                Tables\Columns\ImageColumn::make('hijab.thumbnail'),
